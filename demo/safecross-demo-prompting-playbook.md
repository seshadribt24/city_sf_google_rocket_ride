# SafeCross SF — Demo Build Prompting Playbook

**7 prompts. Copy-paste each one into Claude Code in sequence.**
**Estimated total: 3–4 hours. Run in RocketRide IDE.**

Before starting: get a Gemini API key from https://aistudio.google.com/apikey and a Mapbox token from https://account.mapbox.com/access-tokens/

---

## Prompt 1 of 7: Project skeleton + backend foundation
...
backend/seed_data.py — 10 real SF intersections on the High Injury Network:
...
**✅ Checkpoint:** `curl http://localhost:8000/api/v1/analytics/intersections` returns 10 intersections.

---

## Prompt 2 of 7: Tap simulator
...
**✅ Checkpoint:** `python simulator/seed_historical.py` inserts ~2,000 events. `python simulator/tap_simulator.py --rate 4 --burst` shows colored event logs and `curl http://localhost:8000/api/v1/analytics/summary` shows increasing counts.

---

## Prompt 3 of 7: Gemini AI integration
...
**✅ Checkpoint:** `curl http://localhost:8000/api/v1/ai/insights` returns 3–4 bullet points mentioning specific intersection names and recommended seconds.

---

## Prompt 4 of 7: React dashboard — layout + map
...
**✅ Checkpoint:** Dashboard at `localhost:5173` shows a dark map of SF with 10 teal markers and a stats bar.

---

## Prompt 5 of 7: Event feed + intersection detail
...
**✅ Checkpoint:** Events scroll in live. Clicking a marker shows an hourly bar chart and an AI recommendation card.

---

## Prompt 6 of 7: AI insights panel
...
**✅ Checkpoint:** The AI panel shows 3–4 Gemini-generated insights with specific intersection names and numbers. Typing a question returns a coherent, data-grounded answer.

---

## Prompt 7 of 7: Docker compose + polish
...
**✅ Checkpoint:** `./start.sh` launches the full stack. Dashboard at `localhost:5173` shows live data within 10 seconds.

---

## Troubleshooting prompts

Use these if you hit issues during the build.
...
## Demo script (what to show judges in 60 seconds)

| Time | What to do | What to say |
|------|-----------|-------------|
| 0–10s | Show dashboard with map | "Live ops dashboard monitoring 10 pilot intersections on SF's High Injury Network." |
| 10–20s | Point to event feed as events flow | "A senior just tapped their Clipper card at Market & 5th — 8-second extension granted in under 500ms." |
| 20–30s | Click a map marker | "Drill into Market & 5th — hourly crossing pattern shows peak demand at 8–9 AM." |
| 30–40s | Click AI Recommendation button | "Gemini analyzes patterns and generates timing recommendations for SFMTA engineers." |
| 40–50s | Show AI insights panel | "The AI Safety Analyst recommends a permanent 8-second baseline increase — that eliminates 89% of extension requests." |
| 50–60s | Type a question to the AI | "Engineers can ask questions in natural language. The AI draws on the full dataset for specific, actionable answers." |
